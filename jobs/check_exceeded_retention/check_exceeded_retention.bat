...
cd %audio_src_folder%
echo Heading into the FOR LOOP
echo the Input File is %inputfile%
echo The audio_src_folder is %audio_src_folder%
FOR /F "usebackq tokens=1-4 delims=," %%a IN (%inputfile%) DO (
   echo In for loop %audio_src_folder%%%a >> %Log%
   IF EXIST %%a (
