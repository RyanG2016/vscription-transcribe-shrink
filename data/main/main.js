...
$( function() {
	$( "#date" ).datepicker({
		showAnim: "clip",dateFormat: "dd/mm/yy"
	});
} );
...
$( function() {
	$( "#accord" ).accordion({
		collapsible: true, header: "h3"//,heightStyle: "fill"
		,active:false,activate: function(){
	...
	if(window.hidetxt)
		{
			...
			{				
				setTimeout(function(){ vScriptCallback("hide", document.getElementsByClassName('form-style-5').item(0).offsetHeight, document.getElementsByClassName('form-style-5').item(0).offsetWidth); }, 1200);
...
function getLatestAppVersionNumber(_callback) {
    		$.ajax({url: "https://www.vtexvsi.com/vscription/transcribe/LatestVersion.txt", success: function(result){
