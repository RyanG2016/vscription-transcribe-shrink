...
function c(a){u.removeClass("mdc-linear-progress--closed");z.progress=a/100;100!==a?v.text(a+"%"):(v.text("Complete."),w.style.display="none",x.style.display="inline-block")}function b(){var a=document.createElement("div");a.setAttribute("class","spinner");var b=document.createElement("div"),c=document.createElement("div"),d=document.createElement("div");b.setAttribute("class","bounce1");c.setAttribute("class","bounce2");d.setAttribute("class","bounce3");a.appendChild(b);a.appendChild(c);a.appendChild(d);
return a}function d(a,b){var c=document.createElement("audio");c.setAttribute("preload","metadata");b=URL.createObjectURL(b);c.setAttribute("src",b);c.onloadedmetadata=function(){var b=c.duration;console.log("The duration of file ("+a+") is of: "+b+" seconds");var d=(new Date(1E3*b)).toISOString().substr(11,8).toString();$("#qfile"+a+" td:nth-child(4)").html(d);$("#qfile"+a+" td:nth-child(5)").html(Math.round(b));p++;p===q&&A.removeAttribute("disabled");c.remove()}}function e(a,c,d){var e=document.createElement("tr");
e.setAttribute("id","qfile"+a);var r=document.createElement("td");r.innerHTML=a+1;a=document.createElement("td");a.innerHTML=c;c=document.createElement("td");d=1024>d?d+"bytes":1024<d&&1048576>d?(d/1024).toFixed(1)+"KB":1048576<d?(d/1048576).toFixed(1)+"MB":void 0;c.innerHTML=d;d=document.createElement("td");d.appendChild(b());var t=document.createElement("td");t.appendChild(b());e.appendChild(r);e.appendChild(a);e.appendChild(c);e.appendChild(d);e.appendChild(t);return e}function g(){f.removeChild(f.firstChild);
var a=document.createElement("p");f.appendChild(a);document.querySelector(".submit_btn").setAttribute("disabled","true");document.querySelector(".clear_btn").setAttribute("disabled","true");$(".demo_author").val("");$("#demo_job_type option:selected").html();$(".demo_dictdate").val("yyyy-mm-dd");$("#demo_speaker_type").val(0);$("#demo_comments").val("")}var l=document.getElementById("upload_btn");document.getElementById("upload_btn_lbl");document.getElementById("clear_btn");var A=document.querySelector(".submit_btn"),
w=document.getElementById("cancelUpload"),x=document.getElementById("confirmUpload"),f=document.querySelector(".preview"),n=document.querySelector(".previewModal");document.querySelector("form");new mdc.ripple.MDCRipple(document.querySelector(".clear_btn"));new mdc.ripple.MDCRipple(document.querySelector(".upload_btn_lbl"));new mdc.ripple.MDCRipple(document.querySelector(".submit_btn"));new mdc.ripple.MDCRipple(document.querySelector("#cancelUpload"));new mdc.ripple.MDCRipple(document.querySelector("#confirmUpload"));
var B=document.getElementById("modal"),q=0,p=0;l.style.display="none";l.addEventListener("change",function(){for(;f.firstChild;)f.removeChild(f.firstChild);var a=l.files;if(0===a.length){var c=document.createElement("p");c.textContent="No files currently selected for upload";f.appendChild(c)}else{q=a.length;document.querySelector(".clear_btn").removeAttribute("disabled");c=document.createElement("table");var b=document.createElement("tr"),h=document.createElement("th"),m=document.createElement("th"),
y=document.createElement("th"),g=document.createElement("th"),k=document.createElement("th");h.innerHTML="#";m.innerHTML="File Name";y.innerHTML="Size";g.innerHTML="Duration";k.innerHTML="in secs";b.append(h);b.append(m);b.append(y);b.append(g);b.append(k);c.setAttribute("class","que-files");c.appendChild(b);f.appendChild(c);b=0;a=$jscomp.makeIterator(a);for(h=a.next();!h.done;h=a.next())h=h.value,m=document.createElement("p"),C.includes(h.type)?(c.appendChild(e(b,h.name,h.size)),d(b,h)):(m.textContent=
"File name "+h.name+": Not a valid file type. Update your selection.",f.appendChild(m)),f.appendChild(m),b++}});var z=new mdc.linearProgress.MDCLinearProgress(document.querySelector(".mdc-linear-progress")),u=$(".mdc-linear-progress"),v=$("#progressTxt");document.querySelector(".clear_btn").addEventListener("click",function(a){u.addClass("mdc-linear-progress--closed");files=[];p=q=0;f.removeChild(f.firstChild);a=document.createElement("p");a.textContent="No files currently selected for upload";f.appendChild(a);
document.querySelector(".submit_btn").setAttribute("disabled","true");document.querySelector(".clear_btn").setAttribute("disabled","true")});w.addEventListener("click",function(a){location.reload()});x.addEventListener("click",function(a){location.href="main.php"});var k;$("#upload_form").on("submit",function(b){b.preventDefault();if(validateFields()){B.style.display="block";b.preventDefault();var d=new FormData;b=$("input[type=file]")[0].files;var e=$("#upload_form").serializeArray();$.each(e,function(a,
b){d.append(b.name,b.value)});for(e=0;e<b.length;e++)d.append("file"+e,b[e]),d.append("dur"+e,$("#qfile"+e+" td:nth-child(5)").html()),console.log(b[e]);$.post("data/parts/backend_request.php",{reqcode:60}).done(function(b){b=JSON.parse(b);var e=b.next_job_num;d.append("nextFileID",b.next_job_id);d.append("nextJobNum",e);d.append("reqcode",61);d.append("authorName",$(".demo_author").val());d.append("jobType",$("#demo_job_type option:selected").html());d.append("dictDate",$(".demo_dictdate").val());
