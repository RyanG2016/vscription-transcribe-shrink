...
$jscomp.propertyToPolyfillSymbol[d],$jscomp.defineProperty(e,d,{configurable:!0,writable:!0,value:b})))};$jscomp.polyfill("Array.prototype.find",function(a){return a?a:function(a,c){return $jscomp.findInternal(this,a,c).v}},"es6","es3");var g_fileName,dataTbl;$(document).tooltip({items:":not(#report_ifr,#TypistName, #jobNo, .tooltip)"});
...
$(document).ready(function(){function a(){if(!isConnected||isConnecting)isConnecting=!0,h(connecting),f=new WebSocket("ws://localhost:8001"),f.onopen=b,f.onmessage=e,f.onerror=onerror,f.onclose=c}function b(){isConnected=!0;isConnecting=!1;h(connected);f.send("Transcribe Client Connected.")}function c(){isConnecting=isConnected=!1;firstLaunch?(firstLaunch=!1,h(not_running)):h(not_connected)}function e(a){a=a.data.toString();switch(a){case "play":d(!0);break;case "pause":d(!1);break;case "rw":AblePlayerInstances[0].handleRewind();
break;case "ff":AblePlayerInstances[0].handleFastForward();break;default:a.substring(0,7)===versionCheck&&getLatestAppVersionNumber(a.substring(7),checkVersions)}}function d(a){""!==AblePlayerInstances[0].media.src&&(a?AblePlayerInstances[0].playMedia():AblePlayerInstances[0].pauseMedia())}var g=document.querySelector("form");window.addEventListener("load",a,!1);var f;statusTxt=$("#statusTxt");$(document).ready(function(){$("#send").on("click",function(a){a=$("#txt").val();f.send(a)})});window.addEventListener("unload",
...
{reqcode:80,args:JSON.stringify(b)}).done(function(a){}),clear(),2===d?(loadingTitle.text("Done"),loadingSub.text("Job "+e+" suspended")):3===d?(loadingTitle.text("Done"),loadingSub.text("Job "+e+" marked as complete")):(loadingTitle.text("Done"),loadingSub.text("Job "+e+" updated successfully")),loadingConfirmBtn.css("display","")):(alert("Error Saving Job. Please contact support - "+b+"\n We will attempt to save the text contents to your clipboard if there is any"),tinyMCE.activeEditor.selection.select(tinyMCE.activeEditor.getBody()),
tinyMCE.activeEditor.execCommand("Copy"),clear(),l.style.display="none")})})},c=new FormData,e=$("#jobNo").val().trim(),d=3;switch(a){case "saveBtn":d=3;break;case "suspendBtn":d=2}loadingSub.text("Saving "+e+" data");loadingTitle.text("Please wait..");loadingConfirmBtn.css("display","none");l.style.display="block";$.post("data/parts/backend_request.php",{reqcode:11,args:JSON.stringify({file_id:currentFileID})}).done(function(a){b(a)})}})});$(function(){$("#date").datepicker({showAnim:"clip",dateFormat:"d-M-yy"})});
...
$(document).ready(function(){$("#loadBtn");$("#completeBtn");$("#completeBtn").click(function(){""===!tinymce.get("report").getContent()?validateForm():completePlayer()});$("#loadBtn").click(function(){performClick("fileLoadDiag")});window.hidetxt=!0;$("#control a").click(function(){hideShowForm()});$("#pop").click(function(){openWindowWithPost(AblePlayerInstances[0].media.src,AblePlayerInstances[0].seekBar.position)})});
function openWindowWithPost(a,b){AblePlayerInstances[0].pauseMedia();var c=document.getElementById("modalPlayerForm");c.src.value=a;c.seek.value=b;window.open("","modalPlayer","toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=500,width=500,height=135");c.submit()}
function completePlayer(){var a=$("#loadBtn"),b=$("#completeBtn"),c=AblePlayerInstances[0].media.src.split("/").pop();$(".pop").css("display","none");clearTempAudio(c);AblePlayerInstances[0].seekTo(0);AblePlayerInstances[0].media.pause();setTimeout(function(){AblePlayerInstances[0].media.removeAttribute("src");AblePlayerInstances[0].media.load()},300);a.removeClass("noHover");$("#loadBtn").html('<i class="fas fa-cloud-upload-alt"></i>&nbsp;Load');a.find("i").show();b.addClass("noHover");b.addClass("button");
b.removeClass("button-green")}
function hideShowForm(){window.hidetxt?(window.hidetxt=!1,$("#audio-td").attr("align","center"),$("#fix-td").css("display","none"),$("#demo-td legend").css("display","none"),$("#mainlogo-td").css("display","none"),$("fieldset").slideUp(400,function(){$(".form-style-5").css("min-width","500px");$(".form-style-5").css("width","500px")}),$("#saveBtn").css("display","none"),$(".button-red").css("display","none"),$("#control a").html("Show Text Area")):(window.hidetxt=!0,$("#audio-td").attr("align","right"),
$("#help-td").attr("align","right"),$("#fix-td").css("display",""),$("#demo-td legend").css("display",""),$("#mainlogo-td").css("display",""),$("fieldset").slideDown(400),$(".form-style-5").css("width","90%"),$(".form-style-5").css("min-width","860px"),$("#saveBtn").css("display",""),$(".button-red").css("display",""),$("#control a").html("Hide Text Area"))}
function performClick(a){if((a=document.getElementById(a))&&document.createEvent){var b=document.createEvent("MouseEvents");b.initEvent("click",!0,!1);a.dispatchEvent(b)}}function chooseJob(){getTransJobList(addRowHandlers)}function jobLoadLookup(a){$.post("data/parts/backend_request.php",{reqcode:7,args:JSON.stringify({file_id:a})}).done(function(a){loadIntoPlayer(a)})}function decodeHtml(a){var b=document.createElement("textarea");b.innerHTML=a;return b.value}
...
AblePlayerInstances[0].seekBar.position-rewindAmountOnPause?AblePlayerInstances[0].seekTo(AblePlayerInstances[0].seekBar.position-rewindAmountOnPause):AblePlayerInstances[0].seekTo(0)}}).catch(function(a){});AblePlayer.prototype.onMediaNewSourceLoad=function(){if(2===b.job_status||1===b.job_status)AblePlayerInstances[0].playMedia(),AblePlayerInstances[0].pauseMedia(),AblePlayerInstances[0].seekTo(b.last_audio_position-rewindAmountOnPause);$(".pop").css("display","inline")};AblePlayerInstances[0].onMediaPause=
function(){0<AblePlayerInstances[0].seekBar.position-rewindAmountOnPause?AblePlayerInstances[0].seekTo(AblePlayerInstances[0].seekBar.position-rewindAmountOnPause):AblePlayerInstances[0].seekTo(0)};modal.style.display="none"}
function validateForm(){var a=$('input[name="jobNo"]'),b=$('input[name="authorName"]'),c=$('input[name="TypistName"]'),e=$('input[name="jobType"]'),d=$('input[name="DateDic"]'),g=$('input[name="DateTra"]'),f=!0;""==a.val().trim()&&(showValidate(a),f=!1);""==b.val().trim()&&(showValidate(b),f=!1);""==c.val().trim()&&(showValidate(c),f=!1);""==e.val().trim()&&(showValidate(e),f=!1);""==d.val().trim()&&(showValidate(d),f=!1);""==g.val().trim()&&(showValidate(g),f=!1);return f}$(".validate-form input").each(function(){$(this).focus(function(){hideValidate(this)})});
function showValidate(a){$(a).addClass("alert-validate")}function hideValidate(a){$(a).removeClass("alert-validate")}function getLatestAppVersionNumber(a,b){$.ajax({url:baseURL+"LatestVersion.txt",success:function(c){console.log("latestversion: "+c);b(c.split("-")[0].trim(),a)}})}function checkVersions(a,b){a>b?checkBrowser(!0,a):checkBrowser(!1,a)}
function checkBrowser(a,b){a?($("#updated_version_bar span").html(b),$("#updated_version_bar a").attr("href","controller.php"),$("#updated_version_bar").slideDown("normal","easeInOutBack")):$("#updated_version_bar").slideUp()}
function getTransJobList(a){var b=$(".jobs_tbl");$.post("data/parts/backend_request.php",{reqcode:9}).done(function(c){var e=JSON.parse(c);if(e.error)return b.html(e.data),!0;b.html(e.data);new mdc.dataTable.MDCDataTable(document.querySelector(".mdc-data-table"));dataTbl=$(".jobs_tbl");dataTbl.on("init.dt",function(){$(".cTooltip").hasClass("tooltipstered")||$(".cTooltip").tooltipster({animation:"grow",theme:"tooltipster-punk",arrow:!0})});dataTbl=dataTbl.DataTable({lengthChange:!1,searching:!1,lengthMenu:!1,
pageLength:7,destroy:!0});dataTbl.on("draw",function(){$(".cTooltip").hasClass("tooltipstered")||$(".cTooltip").tooltipster({animation:"grow",theme:"tooltipster-punk",arrow:!0})});setTimeout(function(){a(e.error)},1E3)})}function addRowHandlers(a){if(!a){var b=$(".jobs_tbl").DataTable();$(".jobs_tbl tbody").on("click","tr",function(){var a=b.row(this).id();jobLoadLookup(a);$(".textarea-holder textarea").getNiceScroll().remove()})}}
