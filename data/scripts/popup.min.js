...
new mdc.ripple.MDCRipple(document.querySelector("#suspendBtn"));new mdc.ripple.MDCRipple(document.querySelector("#discardBtn"));new mdc.ripple.MDCRipple(document.querySelector("#loadBtn"));new mdc.ripple.MDCRipple(document.querySelector("#loadingConfirm"));window.addEventListener("load",b,!1);statusTxt=$("#statusTxt");window.addEventListener("unload",disconnect,!1);var d=function(a){statusTxt.html(a);switch(a){case connected:statusTxt.css("color",greenColor);break;case not_running:case not_connected:case connecting:statusTxt.css("color",
orangeColor),$("#reconnect").on("click",function(a){b()})}};$.ajaxSetup({cache:!1});document.getElementById("modal");var k=document.getElementById("modalLoading");loadingConfirmBtn=$("#loadingConfirm");loadingSub=$("#modalLoading .modal-content p i");loadingTitle=$("#modalLoading .modal-content h2");loadingConfirmBtn.on("click",function(){location.reload()});$("#loadBtn").on("click",function(){jobPickerWindow=window.open("job_picker.php","modalPicker","toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=500,width=1000,height=500")});
$("#switchBackBtn").click(function(){postToParent()});$("#discardBtn").click(function(){u()});$("#saveBtn").click(function(){h(!0)});$("#suspendBtn").click(function(){h(!1)});$.post(backend_url,{reqcode:204}).done(function(a){console.log("session saved id is "+a);a?"0"===a?switchUI(!1):loadID(a):switchUI(!1)})});
...
function switchUI(b){b?($(".title-default-text").css("display","none"),$(".title-text-holder").css("display",""),$(".loaded").css("display",""),$(".not-loaded").css("display","none")):($(".title-default-text").css("display","inline-block"),$(".title-text-holder").css("display","none"),$(".loaded").css("display","none"),$(".not-loaded").css("display",""));$(".overlay").css("display","none")}function postToParent(){disconnect();window.opener.switchBack()}
