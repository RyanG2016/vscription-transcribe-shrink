...
// options being used
    ...
    $("#edit").on("click", function() {
        if(editing)
        {
            editing = false;
            $(this).html("edit");
            toggleEditingBillingInfo(false);
        }else{
            editing = true;
            $(this).html("save");
            toggleEditingBillingInfo(true);
        }
    });

    function toggleEditingBillingInfo(editing)
    {
        if(editing)
        {
...
//On Input Change Events
    name.addEventListener('input', function () {
        if (name.value.length == 0) {
            document.getElementById('svgname').innerHTML = fname.html() + " " + lname.html();
            document.getElementById('svgnameback').innerHTML = fname.html() + " " + lname.html();
        } else {
...
/*
    ...
    cardnumber_mask.on('accept', function () {
        // console.log("card number " +cardnumber_mask.value + " | " +cardnumber_mask.value.length);
...
//On Focus Events
    ...
    console.log(securitycode_mask.masked.isComplete , cardnumber_mask.masked.isComplete , securitycode_mask.masked.isComplete)
    if(securitycode_mask.masked.isComplete && cardnumber_mask.masked.isComplete && securitycode_mask.masked.isComplete){
        payBtn.removeAttr("disabled");
    }
});
