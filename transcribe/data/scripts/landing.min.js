...
roleBox.selectpicker("refresh");roleIsset?changeLoading(!1):1==accessesGlobal.length?1==rolesGlobal.length?c(rolesGlobal[0].acc_id,rolesGlobal[0].acc_role):(chooseJobModal.modal("show"),changeLoading(!1)):(0!=accessesGlobal.length&&chooseJobModal.modal("show"),changeLoading(!1))}})}loading=$("#overlay");changeLoading(!0);(new EnjoyHint({})).run();loadingText=$("#loadingText");accNameInput=$("#accNameTxt");currentRole=$("#currentRole");currentAccName=$("#currentAccountName");updateRoleModalBtn=$("#updateRoleBtn");
closeModalBtn=$("#closeModalBtn");changeRoleBtn=$("#changeRoleBtn");modalHeaderTitle=$("#modalHeaderTitle");setDefaultRoleBtn=$("#setDefaultRoleBtn");accessId=$("#accessId");accountBox=$("#accountBox");roleBox=$("#roleBox");chooseJobModal=$("#modal");closeModalBtn.on("click",function(a){chooseJobModal.modal("hide")});changeRoleBtn.on("click",function(a){setModalUI(!1);chooseJobModal.modal("show")});accNameInput.keyup(function(){b()});$("#createAdminAccBtn").on("click",function(a){b()&&(changeLoading(!0,
"Creating account..."),$("#createAccModal").modal("hide"),a=new FormData,a.append("acc_name",accNameInput.val()),$.ajax({type:"POST",url:CREATE_ACC_URL,data:a,processData:!1,contentType:!1,success:function(){loadingText.html("Account created!, redirecting..");setTimeout(function(){location.reload()},1E3)},error:function(a){changeLoading(!1);$.confirm({title:"Error",content:a.responseJSON.msg,buttons:{confirm:{btnClass:"btn-red"}}})}}))});setDefaultRoleBtn.on("click",function(a){setModalUI(!0);chooseJobModal.modal()});
(function(a){a=void 0===a?0:a;var b="";a&&(b="&type="+a);$.ajax({url:ROLES_COUNT_FOR_ACCOUNT_URL+b,method:"GET",success:function(a){typistAccessCount=a.count;$("#typistCount").html(typistAccessCount);0<typistAccessCount?($("#typist0").remove(),$("#alertT0").remove()):($("#typist1").remove(),$("#alertT1").remove())}})})(3);updateRoleModalBtn.on("click",function(a){updateRoleModalBtn.attr("disabled","disabled");changeLoading(!0,"Setting role, please wait..");a=new FormData;a.append("acc_id",accountBox.selectpicker("val"));
a.append("acc_role",roleBox.selectpicker("val"));$.ajax({type:"POST",url:setDefaultModal?SET_DEFAULT_ACCESS_URL:ACCESS_URL,processData:!1,data:convertToSearchParam(a),success:function(a){chooseJobModal.modal("hide");location.reload()},error:function(a){changeLoading(!1);$.confirm({title:"Error",content:a.responseJSON.msg,buttons:{confirm:{btnClass:"btn-red",action:function(){updateRoleModalBtn.removeAttr("disabled");return!0}}}})}})});$.ajax({url:ACCESS_URL,method:"GET",success:function(a){accessesGlobal=
a;var b=document.getElementById("accountBox");a=$jscomp.makeIterator(a);for(var c=a.next();!c.done;c=a.next())c=c.value,-1==accountsArray.indexOf(c.acc_id)&&(accountsArray.push(c.acc_id),b.innerHTML+="<option value='"+c.acc_id+"'>"+c.acc_name+"</option>");accountBox.selectpicker({liveSearch:!0,liveSearchPlaceholder:"Choose Account"});d(accountBox.selectpicker("val"));accountBox.on("changed.bs.select",function(a,b,c,e){changeLoading(!0,"Loading roles..");d(accountBox.selectpicker("val"))})}})});
