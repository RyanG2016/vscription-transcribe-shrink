(function(b){})(jQuery);var dataTbl,jobsDT,jobsDTRef,totalDur=0,totalTrDur=0;
...
{data:"file_date_dict"},{data:"job_upload_date"},{data:"audio_length",render:function(a,b,c){totalDur+=parseInt(a);if(1==c.file_status||2==c.file_status||0==c.file_status)totalTrDur+=parseInt(a);return(new Date(1E3*a)).toISOString().substr(11,8)}},{data:"file_status_ref"},{data:"file_transcribed_date"},{data:"text_downloaded_date"},{data:"times_text_downloaded_date",render:function(a,b,c){return 3==c.file_status?"<a id='view-icon' class=\"material-icons view-icon\">visibility</a> <a class=\"material-icons download-icon\">cloud_download</a> <span class='times-downloaded'>+"+
a+"</span>":""}}],initComplete:function(){this.api().columns([0,3,4,5,7,8]).every(function(){var a=this;$("input",this.footer()).on("keyup change clear",function(){a.search()!==this.value&&a.search(this.value).draw()})});this.api().columns([1,2,6]).every(function(){var a=this,b=$('<select><option value=""></option></select>').appendTo($(a.footer()).empty()).on("change",function(){var c=$.fn.dataTable.util.escapeRegex($(this).val());a.search(c?"^"+c+"$":"",!0,!1).draw()});a.data().unique().sort().each(function(a,
d){b.append('<option value="'+a+'">'+a+"</option>")})})}});$("#jobs-tbl tfoot th:eq(0),#jobs-tbl tfoot th:eq(3),#jobs-tbl tfoot th:eq(4),#jobs-tbl tfoot th:eq(7),#jobs-tbl tfoot th:eq(8),#jobs-tbl tfoot th:eq(5)").each(function(){$(this).html('<input class="dt-search" type="text"/>')});b.addEventListener("click",function(a){jobsDTRef.ajax.reload()});jobsDT.on("draw.dt",function(){$(".download-icon").click(function(){var a=$(this).parent().parent().attr("id");download(a)});$(".view-icon").click(function(){var a=
$(this).parent().parent().attr("id");view(a)});$(".cTooltip").hasClass("tooltipstered")||$(".cTooltip").tooltipster({animation:"grow",theme:"tooltipster-punk",arrow:!0});$("#tjd").html("Total Jobs Length: "+(new Date(1E3*totalDur)).toISOString().substr(11,8));$("#cbm").html("Current Backlog Minutes: "+(new Date(1E3*totalTrDur)).toISOString().substr(11,8))})});
function download(b){$.post("data/parts/backend_request.php",{reqcode:17,args:JSON.stringify({file_id:b})}).done(function(b){window.open("./download.php?down="+b.toString(),"_blank").focus();jobsDTRef.ajax.reload()})}
