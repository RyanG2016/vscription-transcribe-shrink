...
a+"</span>":""}}],initComplete:function(){this.api().columns().every(function(){var a=this,b=$('<select><option value=""></option></select>').appendTo($(a.footer()).empty()).on("change",function(){var c=$.fn.dataTable.util.escapeRegex($(this).val());a.search(c?"^"+c+"$":"",!0,!1).draw()});a.data().unique().sort().each(function(a,d){b.append('<option value="'+a+'">'+a+"</option>")})})}});b.addEventListener("click",function(a){jobsDTRef.ajax.reload()});jobsDT.on("draw.dt",function(){$(".download-icon").click(function(){var a=
$(this).parent().parent().attr("id");download(a)});$(".view-icon").click(function(){var a=$(this).parent().parent().attr("id");view(a)});$(".cTooltip").hasClass("tooltipstered")||$(".cTooltip").tooltipster({animation:"grow",theme:"tooltipster-punk",arrow:!0});$("#tjd").html("Total Jobs Length: "+(new Date(1E3*totalDur)).toISOString().substr(11,8));$("#cbm").html("Current Backlog Minutes: "+(new Date(1E3*totalTrDur)).toISOString().substr(11,8))})});
