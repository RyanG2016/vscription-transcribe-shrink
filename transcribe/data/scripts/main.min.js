(function(b){})(jQuery);var dataTbl,jobsDT,jobsDTRef,totalDur=0;
...
{data:"file_date_dict"},{data:"job_upload_date"},{data:"audio_length",render:function(a){totalDur+=parseInt(a);return(new Date(1E3*a)).toISOString().substr(11,8)}},{data:"file_status_ref"},{data:"file_transcribed_date"},{data:"text_downloaded_date"},{data:"times_text_downloaded_date",render:function(a,b,c){return 3==c.file_status?"<a id='view-icon' class=\"material-icons view-icon\">visibility</a> <a class=\"material-icons download-icon\">cloud_download</a> <span class='times-downloaded'>+"+a+"</span>":
""}}],initComplete:function(){this.api().columns().every(function(){var a=this,b=$('<select><option value=""></option></select>').appendTo($(a.footer()).empty()).on("change",function(){var b=$.fn.dataTable.util.escapeRegex($(this).val());a.search(b?"^"+b+"$":"",!0,!1).draw()});a.data().unique().sort().each(function(a,d){b.append('<option value="'+a+'">'+a+"</option>")})})}});b.addEventListener("click",function(a){totalDur=0;jobsDTRef.ajax.reload()});jobsDT.on("draw.dt",function(){$(".download-icon").click(function(){var a=
$(this).parent().parent().attr("id");download(a)});$(".view-icon").click(function(){var a=$(this).parent().parent().attr("id");view(a)});$(".cTooltip").hasClass("tooltipstered")||$(".cTooltip").tooltipster({animation:"grow",theme:"tooltipster-punk",arrow:!0});$("#tjd").html("Total Jobs Duration: "+(new Date(1E3*totalDur)).toISOString().substr(11,8))})});
function download(b){$.post("data/parts/backend_request.php",{reqcode:17,args:JSON.stringify({file_id:b})}).done(function(b){window.open("./download.php?down="+b.toString(),"_blank").focus();totalDur=0;jobsDTRef.ajax.reload()})}
function view(b){$.post("data/parts/backend_request.php",{reqcode:17,args:JSON.stringify({file_id:b})}).done(function(b){window.open("./view.php?down="+b.toString(),"popUpWindow","height=800,width=750,left=100,top=100,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes").focus();totalDur=0;jobsDTRef.ajax.reload()})}
function htmlEncodeStr(b){return b.replace(/&/g,"&amp;").replace(/>/g,"&gt;").replace(/</g,"&lt;").replace(/"/g,"&quot;").replace(/'/g,"&lsquo;")};
