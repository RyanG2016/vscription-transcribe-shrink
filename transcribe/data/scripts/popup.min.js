...
break;case "ff":AblePlayerInstances[0].handleFastForward();break;default:a.substring(0,7)===versionCheck?p(a.substring(7),q):a.substring(0,8)}}function h(a){""!==AblePlayerInstances[0].media.src&&(a?AblePlayerInstances[0].playMedia():AblePlayerInstances[0].pauseMedia())}function k(a){var d=new FormData,b=$(".jobNo").html().trim(),c=5;a||(c=2);loadingSub.text("Saving "+b+" data");loadingTitle.text("Please wait..");loadingConfirmBtn.css("display","none");f.style.display="block";a=Math.floor(AblePlayerInstances[0].seekBar.position).toString();
d.append("audio_length",Math.round(AblePlayerInstances[0].seekBar.duration));d.append("last_audio_position",a);d.append("file_status",c);d.append("set_role",3);$.ajax({type:"POST",url:files_api+currentFileID,data:d,processData:!1,contentType:!1,success:function(a){a={mailtype:10,usertype:2};3===c&&$.post("data/parts/backend_request.php",{reqcode:80,args:JSON.stringify(a)}).done(function(a){});g();2===c?(loadingTitle.text("Done"),loadingSub.text("Job "+b+" suspended")):3===c?(loadingTitle.text("Done"),
loadingSub.text("Job "+b+" marked as complete")):(loadingTitle.text("Done"),loadingSub.text("Job "+b+" updated successfully"));loadingConfirmBtn.css("display","")},error:function(a){alert("Error Saving Job. Please contact support - ${data}\n We will attempt to save the text contents to your clipboard if there is any");tinyMCE.activeEditor.selection.select(tinyMCE.activeEditor.getBody());tinyMCE.activeEditor.execCommand("Copy");g();f.style.display="none"}})}function r(){var a=2;0===currentFileData.job_status&&
(a=0);$.post("data/parts/backend_request.php",{reqcode:16,args:JSON.stringify({file_id:currentFileID,new_status:a})}).done(function(a){});g()}function g(){$(".title-default-text").css("display","inline-block");$(".title-text-holder").css("display","none");currentFileID=0;t();switchUI(!1)}function t(){AblePlayerInstances[0].media.src.split("/").pop();AblePlayerInstances[0].seekTo(0);AblePlayerInstances[0].media.pause();setTimeout(function(){AblePlayerInstances[0].media.removeAttribute("src");AblePlayerInstances[0].media.load()},
300)}function u(){$.confirm({title:"Discard Form?",content:"Are you sure do you want to discard current job?",buttons:{confirm:{btnClass:"btn-red",action:function(){r();return!0}},cancel:function(){}}})}function p(a,b){$.ajax({url:baseURL+"LatestVersion.txt",success:function(c){console.log("latestversion: "+c);b(c.split("-")[0].trim(),a)}})}function q(a,c){a>c?l(!0,a):l(!1,a)}function l(a,c){a?($("#updated_version_bar span").html(c),$("#updated_version_bar a").attr("href","controller.php"),$("#updated_version_bar").slideDown("normal",
"easeInOutBack")):$("#updated_version_bar").slideUp()}$(".overlay").css("display","");$(".loaded").css("display","none");new mdc.ripple.MDCRipple(document.querySelector("#saveBtn"));new mdc.ripple.MDCRipple(document.querySelector("#suspendBtn"));new mdc.ripple.MDCRipple(document.querySelector("#discardBtn"));new mdc.ripple.MDCRipple(document.querySelector("#loadBtn"));new mdc.ripple.MDCRipple(document.querySelector("#loadingConfirm"));window.addEventListener("load",c,!1);statusTxt=$("#statusTxt");
var e=function(a,b){b=void 0===b?!1:b;statusTxt.html(a);switch(b){case !0:statusTxt.css("color",greenColor);break;case !1:statusTxt.css("color",orangeColor),$("#reconnect").on("click",function(a){c()})}};$.ajaxSetup({cache:!1});var f=document.getElementById("modalLoading");loadingConfirmBtn=$("#loadingConfirm");loadingSub=$("#modalLoading .modal-content p i");loadingTitle=$("#modalLoading .modal-content h2");loadingConfirmBtn.on("click",function(){f.style.display="none"});$("#loadBtn").on("click",
function(){jobPickerWindow=window.open("job_picker.php","modalPicker","toolbar=yes,scrollbars=yes,resizable=yes,top=500,left=500,width=1000,height=500")});$("#switchBackBtn").click(function(){postToParent()});$("#discardBtn").click(function(){u()});$("#saveBtn").click(function(){k(!0)});$("#suspendBtn").click(function(){k(!1)});$.post(backend_url,{reqcode:204}).done(function(a){a?"0"===a?switchUI(!1):loadID(a):switchUI(!1)})});
