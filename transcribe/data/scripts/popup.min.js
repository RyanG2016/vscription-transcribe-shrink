...
function loadID(b){$(".overlay").css("display","");jobPickerWindow&&jobPickerWindow.close();$.get(files_api+b+"?tr",{}).done(function(b){b?loadIntoPlayer(b):(switchUI(!1),$.confirm({title:"Error",content:"Job doesn't exist or you don't have permission to access it.",buttons:{confirm:{btnClass:"btn-green",text:"ok"}}}))})}
function loadIntoPlayer(b){var c=JSON.parse(b);currentFileData=c;currentFileID=c.file_id;$("#jobNo").html(c.job_id);$("#author").html(c.file_author);$("#jobType").html(c.file_work_type);AblePlayerInstances[0].media.src=audioTempFolder+c.tmp_name;AblePlayer.prototype.onMediaNewSourceLoad=function(){2===c.job_status||1===c.job_status?AblePlayerInstances[0].seekTo(c.last_audio_position-rewindAmountOnPause):AblePlayerInstances[0].seekTo(0);switchUI(!0)};AblePlayerInstances[0].onMediaPause=function(){0<
